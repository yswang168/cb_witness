...
# in the ASP compeition 2015
  ...
  if AllInstances:
    ...
    files = [f for f in files if f.startswith('00') and f.endswith(".asp")]
  else:
  ...
  for f in files:
    ...
    print("python3.7 cb_witness.py -t 7200 -lp -v4 -TT_PYSAT -q %s %s "%(fm_name,flp_name))
    subprocess.call(["python3.7", "cb_witness.py", "-t 7200", "-lp", "-v4", "-TT_PYSAT", "-q", flp_name, fm_name])
    fm.close()
    flp.close()
